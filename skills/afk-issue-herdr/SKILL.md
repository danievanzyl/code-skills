...
- **`--disallowedTools "WebSearch WebFetch mcp__*"`** — bare tool names, so those tools are *removed from the pane agent's context* (deny wins even under `auto`; an allow-list would not bite). Shrinks blast radius by cutting network + the human's inherited MCP servers (Atlassian/pencil — an AFK coder has no business there). `Task` is **deliberately kept** (the Engineer seam — see Sub-agent nesting); this matches the agent defs' `tools:` set, which is also web/MCP-free.
...
- **External references via `gh-search-researcher`, conditionally.** ([ADR-0006](../../docs/adr/0006-herdr-runner-gh-reference-collection.md)) If you judge the issue needs external, cross-repo reference material — self-contained in-repo work skips this — nest `agentic-platform:gh-search-researcher` via your own `Task` tool (you already have `Task`; `--plugin-dir` loads the agent type, no infra change) instead of traversing local disk for it. Search is **open-ended and unbounded across all of GitHub** — no org scoping; hand it the issue and let it decide relevance. In the invocation prompt, coerce it to return relevant **code excerpts + URLs + why-relevant**, not its default URL-and-summary research report. Results land **inline in your context** via ordinary `Task` return behaviour — never write them to disk (protects the Reviewer's clean-tree check and the sandbox write-boundary). This is external refs only; your own worktree reading via local filesystem is unchanged.
