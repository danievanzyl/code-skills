...
- **Name it before calling `agent start` — herdr agent names are a global namespace, not scoped per workspace/tab** (same caveat as `afk-issue-herdr`'s Isolation step 2; an unqualified name collides with any other agent on the machine). Resolve once: `NAME=handoff-<tab-id>`, using whichever tab id the branch below targets (the matched pane's tab id, or the new tab id from `workspace create`), sanitizing `:`→`-`. Use `$NAME` at every call site below (`agent start`, `agent wait`, and Delivery).
